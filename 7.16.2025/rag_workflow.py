...
def extract_docs_from_files(files) -> List[Document]:
    # print(f"extract_docs_from_files: {files}")
    extracted_docs = []
    filenames = []
...
def create_vector_store_from_docs(extracted_chunks: List[Document]) -> Chroma:
    ...
    if vector_store is not None:
        print(f"Vector store already exists, skipping creation.")
...
# def run_workflow(messages: List[AnyMessage]):
@cl.step(name="Processing files...")
async def run_workflow(messages: List[AnyMessage], files: List[Document]):
    """Run the workflow with the provided messages."""
    global extracted_chunks, vector_store
    extracted_docs, filenames = await make_async(extract_docs_from_files)(
        # paths=["pdf/12_10_2024_Hemoglobin_and_Hematocrit.pdf", "pdf/12_7_2024_Urinalysis.pdf"])
        files)

    # extracted_chunks.extend(extracted_docs)

    vector_store = await make_async(create_vector_store_from_docs)(
        extracted_chunks=extracted_docs)
    # print(f"extracted_chunks: {extracted_chunks}")
    return extracted_docs, filenames
    # config = {"configurable": {"thread_id": "2"}}

    # state = await graph.ainvoke({"messages": messages}, config=config)
    # return state["all_retrieved_docs"]


# run_workflow([
#     HumanMessage(content="What were the hemoglobin and hematocrit levels?"),
# ])
