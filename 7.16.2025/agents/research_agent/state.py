...
class ResearchContext(BaseModel):
    """
    Structured output model for the Research Agent in an information retrieval and synthesis system.

    This model encapsulates the agent's reasoning and analysis of research context,
    including key findings, risk assessment, and educational insights. It is designed
    to help users understand research topics, identify important patterns, and
    determine the relevance and reliability of information sources.
    """
    ...
    sources: List[str] = Field(
        default_factory=list,
        description="List of sources or references used in the analysis."
    )


@dataclass
class State(AgentState):
    """Represents the complete state of the agent, extending AgentState with additional attributes.
    ...
    research_context: ResearchContext = Field(
        ...
        description="Contextual information gathered by the research agent."
    )
    # Common examples include:
    # attached_files: Annotated[List[Document],
    #                           operator.add] = field(default_factory=list)
    # document_context: Annotated[List[Any],
    #                             operator.add] = field(default_factory=list)
